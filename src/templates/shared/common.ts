...
import type {
  BulletStyle, Customize, PersonalInfo, ReadCustomize, Section, SectionType, TemplateMeta,
} from '../../types';

const BULLET_CHARS: Record<BulletStyle, string> = { dot: '•', dash: '–', arrow: '→', none: '' };
const PHOTO_SIZES = { small: 64, medium: 80, large: 96 } as const;

// Shallow-per-group merge: template defaults under user overrides.
function mergeWithDefaults(customize: Customize, defaults: Customize): Customize {
  const out: Record<string, unknown> = { ...defaults, ...customize };
  const c = customize as Record<string, unknown>;
  ...
  return out as Customize;
}

// Derives every commonly-needed value from a resolved customize object.
export function readCustomize(customize: Customize = {}, meta?: TemplateMeta): ReadCustomize {
...
const SECTION_KIND: Partial<Record<SectionType, SectionKind>> = {
...
export function sectionKind(type: SectionType): SectionKind {
  return SECTION_KIND[type] || (type as SectionKind);
}

export function fullNameOf(personal: PersonalInfo): string {
  return [personal.firstName, personal.lastName].filter(Boolean).join(' ');
}

export function visibleSectionsOf(sections: Section[]): Section[] {
  return sections
    .filter(s => s.visible)
    .map(s => ({ ...s, entries: s.entries.filter(e => e.visible) } as Section))
