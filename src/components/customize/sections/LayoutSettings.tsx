...
import {
  GripVertical, User, Briefcase, Brain, GraduationCap, Globe, Award, FileText, AlignJustify,
...
export default function LayoutSettings({ customize, updateCustomize }: CustomizeSectionProps) {
  ...
  const columns = allColumns.filter(c => (supportedColumns as string[]).includes(c.id));
          ...
          <div className="cz-columns-picker">
            {columns.map(col => (
              <button
                ...
                onClick={() => updateCustomize('layout', { columns: col.id as 'one' | 'two' })}
              >
