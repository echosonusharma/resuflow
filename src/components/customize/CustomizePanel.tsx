...
export default function CustomizePanel({ setView }: { setView: (view: string) => void }) {
  ...
  const scrollLockTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      entries => {
        ...
        let bestRatio = 0;
        visibility.forEach((ratio: number, id: string) => {
          if (ratio > bestRatio) { bestRatio = ratio; bestId = id; }
        });
        if (bestId !== null && bestRatio > 0) {
          setActiveSection((bestId as string).replace('cz-', ''));
        }
      },
