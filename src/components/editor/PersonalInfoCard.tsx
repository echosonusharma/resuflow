...
type FieldKey = 'title' | 'email' | 'phone' | 'location' | 'linkedin' | 'website' | 'github' |
  'nationality' | 'dateOfBirth' | 'visa' | 'passportId' | 'availability';
...
export default function PersonalInfoCard() {
  ...
  const order = useMemo(() => {
    const stored: string[] = Array.isArray(personal._fieldOrder) && personal._fieldOrder.length
      ? personal._fieldOrder.filter((k): k is FieldKey => k in FIELDS)
      : [...DEFAULT_ORDER];
  ...
  function setOrder(next: string[] | ((prev: string[]) => string[])) {
  ...
  function addField(key: string) {
    updatePersonal({
      ...
      ...(personal[key as keyof PersonalInfo] == null ? { [key]: '' } : {}),
    });
  }
    ...
    <div className="personal-card">
      ...
      <div className="pi-form">
        ...
        {order.map(key => {
          ...
          const f = FIELDS[fkey];
          if (!f) return null;
          const personalVal = personal[fkey as keyof PersonalInfo] as string | undefined;
          const err = touched[fkey] && f.validate ? f.validate(personalVal || '') : null;
              ...
              <div className="pi-field">
                ...
                <div className="pi-input-wrap">
                  <input
                    ...
                    type={f.type || 'text'}
                    value={personalVal || ''}
                    onChange={e => set(fkey as keyof PersonalInfo, e.target.value)}
                    onBlur={() => touch(fkey)}
                    placeholder={f.placeholder}
                  />
                  {f.linkChip && personalVal && (
            ...
            <div className="pi-add-details-chips">
              {availableOptional.map(k => (
                <button
                  ...
                  onClick={() => addField(k)}
                >
                  <Plus size={12} /> {FIELDS[k as FieldKey].label}
                </button>
              ))}
            </div>
          </div>
